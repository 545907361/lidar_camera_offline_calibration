...
///前向60度相机回调函数
void CHandler::CallbackRawImage_a_0(const lcm::ReceiveBuffer* recvBuf, const std::string& channelName, const GMSL_IMAGE_ENCODE* msg)
{
    m_mtuRawImageFrame.lock();

    m_msgRawImageFrame_a_0.decode ( recvBuf->data, 0, recvBuf->data_size );


    cv::Mat jpegimage= cv::imdecode(cv::Mat(m_msgRawImageFrame_a_0.gbImageData),CV_LOAD_IMAGE_COLOR);
    cv::cvtColor(jpegimage, jpegimage, cv::COLOR_BGR2RGB);
    jpegimage.copyTo(m_image);
    m_mtuRawImageFrame.unlock();
    m_bGetImage = true;
...
///32线激光雷达回调函数
void CHandler::CallbackRawLidar_32(const lcm::ReceiveBuffer* recvBuf, const std::string& channelName, const RSLIDAR_32_FRAME *msg)
{
    m_mtuRawLidarFrame.lock();

    memcpy(&m_msgRawLidarFrame_32, msg, sizeof(RSLIDAR_32_FRAME));
    ...
    for(size_t i=0; i<32; i++){
        for(size_t j=0; j<m_msgRawLidarFrame_32.gstRingPoints[i].nNumOfPoints; j++){
            points_num++;
            p.x = (float)m_msgRawLidarFrame_32.gstRingPoints[i].gfX[j] * m_msgRawLidarFrame_32.fResolution;
            p.y = (float)m_msgRawLidarFrame_32.gstRingPoints[i].gfY[j] * m_msgRawLidarFrame_32.fResolution;
            p.z = (float)m_msgRawLidarFrame_32.gstRingPoints[i].gfZ[j] * m_msgRawLidarFrame_32.fResolution;
            raw_pc -> push_back(p);
        }
    }
    //std::cout << "get " << points_num << "points" << std::endl;
...
void CHandler::CallbackTransform(const lcm::ReceiveBuffer *recvBuf, const std::string &channelName, const TRANSFORM *msg)
{
    m_mtuTransform.lock();
    memcpy(&m_msgTransform, msg, sizeof(TRANSFORM));

    m_bTransformInited = true;

    if( ClockDiv(10) ){//100ms
        if(m_bGetImage && m_bGetLidarClouds){
            fprintf(stdout, "do pnp action....\n");
            /*
             * 解算程序　do_action()
             */
        }
